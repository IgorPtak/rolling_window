...
SEXP rolling_median_c(SEXP r_data, SEXP r_window_size, SEXP r_min_periods,
  ...
  std::size_t k = read_window_size(r_window_size);
  std::size_t mp = read_min_periods(r_min_periods);
  bool expect_nan = Rf_asLogical(r_expect_nan) == TRUE;

  SlidingMedian sliding_median(k, expect_nan);

  SEXP r_result;
  PROTECT(r_result = Rf_allocVector(REALSXP, n));
  double *output_ptr = REAL(r_result);

  sliding_median.process_batch(input_ptr, static_cast<std::size_t>(n),
                               output_ptr, mp);
...
SEXP rolling_variance_fast_c(SEXP r_data, SEXP r_window_size,
  ...
  SEXP r_result;
  PROTECT(r_result = Rf_allocVector(REALSXP, n));
  smp.variance_batch(input_ptr, static_cast<std::size_t>(n), REAL(r_result),
                     mp);
  UNPROTECT(1);
  return r_result;
}
...
SEXP rolling_skewness_fast_c(SEXP r_data, SEXP r_window_size,
  ...
  SEXP r_result;
  PROTECT(r_result = Rf_allocVector(REALSXP, n));
  smp.skewness_batch(input_ptr, static_cast<std::size_t>(n), REAL(r_result),
                     mp);
  UNPROTECT(1);
  return r_result;
}
...
SEXP rolling_kurtosis_fast_c(SEXP r_data, SEXP r_window_size,
  ...
  SEXP r_result;
  PROTECT(r_result = Rf_allocVector(REALSXP, n));
  smp.kurtosis_batch(input_ptr, static_cast<std::size_t>(n), REAL(r_result),
                     mp);
  UNPROTECT(1);
  return r_result;
}
...
static const R_CallMethodDef CallEntries[] = {
    ...
    {"rolling_median_c", reinterpret_cast<DL_FUNC>(&rolling_median_c), 4},
    ...
    {"rolling_variance_fast_c",
     reinterpret_cast<DL_FUNC>(&rolling_variance_fast_c), 3},
    {"rolling_skewness_fast_c",
     reinterpret_cast<DL_FUNC>(&rolling_skewness_fast_c), 3},
    {"rolling_kurtosis_fast_c",
     reinterpret_cast<DL_FUNC>(&rolling_kurtosis_fast_c), 3},
